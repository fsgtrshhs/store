...
def send_email(smtpHost,port, sendAddr, password, recipientAddrs, subject='', content=''):
    ...
    part = MIMEApplication(open(r'D:\Python课程资料\day13【单元测试】\代码\day13\计算器.html','rb').read())
...
if __name__ == "__main__":
    try:
        ...
        port = 465 #端口号
        sendAddr ='2954789724@qq.com'#发送方地址
        password = input("请输入邮箱授权码：")#手动输入授权码更安全.授权码的获取:打开qq邮箱->设置->账户->开启IMAP/SMTP服务->发送短信->授权码
        recipientAddrs = '2954789724@qq.com;2431320433@qq.com'#接收方可以是多个账户, 用分号分开,send_email()函数中手动设置
